...
def load():
    ...
    project_dir = os.path.dirname(os.path.abspath(args.file))

    # Load resources
    resources = set()

    resources.update(os.listdir(os.path.join(project_dir, "sprites")))
    resources.update(os.listdir(os.path.join(project_dir, "sounds")))
    resources.update(os.listdir(os.path.join(project_dir, "objects")))
    resources.update(os.listdir(os.path.join(project_dir, "tilesets")))
    resources.update(os.listdir(os.path.join(project_dir, "fonts")))
    resources.update(os.listdir(os.path.join(project_dir, "rooms")))
    resources.update(os.listdir(os.path.join(project_dir, "shaders")))
    resources.update(os.listdir(os.path.join(project_dir, "particles")))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="A Python program to calculate statistics about your GameMaker 2 projects.")
    parser.add_argument('file', type=str, help='Path to the GameMaker 2 project file. Should end in .yyp.')

    args = parser.parse_args()

    if os.path.exists(args.file) and args.file.endswith(".yyp"):
        project_name = os.path.splitext(os.path.basename(args.file))[0]
    else:
        print(f'ERROR: Path "{args.file}" is not a valid GameMaker 2 project file.')
        exit()

    files, resources, scripts, enum_names, enum_entries, macros, globalvars = load()

    gms2_stats_viz.launch(project_name, files, resources, scripts, enum_names, enum_entries, macros, globalvars)